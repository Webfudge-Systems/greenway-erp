...
class AuthService {
  ...
  /**
   * Platform super-admin login (Organization Manager app only).
   */
  async platformLogin(email, password) {
    try {
      ...
      if (!response.ok) {
        const errorMessage =
          data?.error?.message || data?.message || (typeof data?.error === 'string' ? data.error : 'Login failed');
        throw new Error(errorMessage);
      }
    ...
    try {
      ...
      const mergeStored = (authUser) => {
        ...
        return {
          ...
          isPlatformAdmin: authUser.isPlatformAdmin ?? prev.isPlatformAdmin,
        };
      };

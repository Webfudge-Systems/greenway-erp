 ...
 *
 * Base URL resolution (same order as `@greenways/auth` and CRM):
 * 1. `NEXT_PUBLIC_API_URL` from `.env.production` / `.env.local` / hosting env (inlined at build time)
 * 2. If unset: production → `https://api.greenwaymobility.in`, development → `http://localhost:1337`
 *
 * All `lib/api/*` services use this instance only — no hardcoded API hosts elsewhere.
 */
const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL ||
  (process.env.NODE_ENV === 'production' ? 'https://api.greenwaymobility.in' : 'http://localhost:1337')

...
export default function PMSidebar({
  ...
  const visiblePmTools = pmTools.filter(
    (item) => !item.module || canReadPM(item.module),
  );
  ...
  const isPmToolActive = (item) => {
    if (item.comingSoonFeature) {
      return (
        ...
        searchParams.get("feature") === item.comingSoonFeature
      );
    }
    if (item.href?.startsWith("/coming-soon")) {
      const feature = item.href.split("feature=")[1]?.split("&")[0];
      return (
        pathname === "/coming-soon" && searchParams.get("feature") === feature
      );
    }
  ...
  const getProjectHealthStyles = (project) => {
    ...
    if (s === "COMPLETED")
      return { icon: "text-emerald-600", label: "Completed" };
      ...
      <div className="shrink-0 px-4 pt-4 pb-3">
        ...
        >
          ...
          {isMobileDrawer ? (
            ...
            >
              <X className="h-5 w-5 text-brand-foreground" />
            </button>
          ) : (
            <button
              type="button"
        ...
        <div className="px-3 pt-3 pb-2">
          ...
          <div
            className={`grid gap-2.5 ${
              !showExpanded ? "grid-cols-1" : "grid-cols-2"
            }`}
          >
                  ...
                  : item.href.startsWith("/coming-soon")
                    ? pathname === "/coming-soon" &&
                      searchParams.get("feature") ===
                        item.href.split("feature=")[1]?.split("&")[0]
                ...
                {loadingProjects ? (
                  ...
                  <div className="space-y-0.5">
                    {displayedProjects.map((project) => {
                      ...
                      const routeActive = pathname.startsWith(
                        `/projects/${slugPath}`,
                      );

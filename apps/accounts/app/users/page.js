...
export default function UsersPage() {
  ...
  useEffect(() => {
    if (!canEditPassword) return undefined
    let cancelled = false
    ;(async () => {
      try {
        const settings = await organizationService.getSecuritySettings()
        if (!cancelled) {
          const min = Number(settings?.passwordMinLength)
          setPasswordMinLength(Number.isFinite(min) && min >= 6 ? min : 8)
        }
      } catch {
        if (!cancelled) setPasswordMinLength(8)
      }
    })()
    return () => {
      cancelled = true
  ...
  const submitEdit = useCallback(async () => {
    ...
    if (editChangePassword) {
      ...
      if (nextPassword.length < passwordMinLength) {
        setEditError(`Password must be at least ${passwordMinLength} characters.`)
        return
      }
    }
            ...
            <>
              ...
              {editChangePassword ? (
                <div className="space-y-1.5">
                  ...
                  <p className="text-xs text-gray-500">
                    Minimum {passwordMinLength} characters. The user will sign in with this password.
                  </p>
                </div>
              ) : null}
            </>

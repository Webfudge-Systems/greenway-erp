...
const { logPlatformActivity } = require('../../../utils/platform-activity-log');
...
module.exports = createCoreService('api::organization.organization', ({ strapi }) => ({
  async createWithOnboarding({ userId, organizationData, appId, moduleIds, userCount, invitedEmails }) {
    ...
    if (subscription) {
      const subWithApp = await strapi.entityService.findOne('api::subscription.subscription', subscription.id, {
        populate: { app: true },
      });
      await logPlatformActivity(strapi, {
        organizationId: organization.id,
        ...
        entity: subWithApp,
        patch: {
          status: subWithApp?.status || 'trial',
          billingCycle: subWithApp?.billingCycle,
        },
      });
    }

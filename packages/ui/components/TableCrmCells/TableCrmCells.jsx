 ...
 * Supports single-line `value` or two-line `primary` / `secondary` (primary may be a React node).
 */
export function TableCellText({
  ...
  const hasStack = primary !== undefined || secondary !== undefined;

  if (hasStack) {
    const primaryNode = primary !== undefined ? primary : value;
    const hasPrimary =
      primaryNode != null &&
      primaryNode !== '' &&
      !(typeof primaryNode === 'string' && primaryNode.trim() === '');
    ...
    return (
      <div className={clsx('min-w-0', maxWidthClass, className)}>
        <div
          className={clsx(
            'truncate text-sm',
            emphasized || typeof primaryNode !== 'string'
              ? 'font-semibold text-gray-900'
              : 'font-medium text-gray-900'
          )}
          title={typeof primaryNode === 'string' ? primaryNode : undefined}
        >
          {hasPrimary ? primaryNode : '—'}
        </div>
        {hasSecondary ? (
          <div className="truncate text-sm text-gray-500">{secondary}</div>
        ) : null}
      </div>
    );
  }
